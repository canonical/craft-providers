...
from craft_providers.bases import buildd, craft_config, errors
...
def test_setup(  # pylint: disable=too-many-arguments
    ...
    fake_process.register_subprocess(
        [*fake_cmd, "test", "-f", "/etc/craft.conf"], returncode=1
    )
    fake_process.register_subprocess(
...
def test_ensure_image_version_compatible_failure(
    ...
    monkeypatch.setattr(
        craft_config,
        "load",
        lambda **kwargs: craft_config.CraftBaseConfig(compatibility_tag="invalid-tag"),
    )

    with pytest.raises(errors.BaseCompatibilityError) as exc_info:
        base_config._ensure_image_version_compatible(  # pylint: disable=protected-access
...
def test_ensure_os_compatible_name_failure(
    ...
    base_config = buildd.BuilddBase(alias=buildd.BuilddBaseAlias.FOCAL)
    monkeypatch.setattr(
        base_config, "_read_os_release", lambda **kwargs: {"NAME": "CentOS"}
    )
    ...
    assert exc_info.value == errors.BaseCompatibilityError(
        "Exepcted OS 'Ubuntu', found 'CentOS'"
    )
...
def test_ensure_os_compatible_version_failure(
    ...
    monkeypatch.setattr(
        base_config,
        "_read_os_release",
        lambda **kwargs: {"NAME": "Ubuntu", "VERSION_ID": "12.04"},
    )
...
def test_read_os_release_failure(
    ...
    with pytest.raises(errors.BaseConfigurationError) as exc_info:
        base_config._read_os_release(  # pylint: disable=protected-access
            executor=fake_executor
        )
...
def test_wait_for_system_ready_timeout(fake_executor, fake_process, alias):
    ...
    with pytest.raises(errors.BaseConfigurationError) as exc_info:
        base_config.wait_until_ready(
            executor=fake_executor, retry_wait=0.01, timeout=time.time()
        )
