...
@pytest.mark.parametrize(
    ...
    [
        (BaseAlias.UBUNTU_BIONIC.value, "core18"),
        (BaseAlias.UBUNTU_FOCAL.value, "core20"),
        (BaseAlias.UBUNTU_JAMMY.value, "core22"),
        (BaseAlias.UBUNTU_KINETIC.value, "kinetic"),
        (BaseAlias.UBUNTU_LUNAR.value, "lunar"),
    ],
)
...
def test_get_image_remote_xenial_error():
    ...
    with pytest.raises(lxd.LXDError) as raised:
        lxd.remotes.get_remote_image(BaseAlias.UBUNTU_XENIAL.value[1])
...
def test_configure_buildd_image_remote(
    ...
    mock_get_remote_image.assert_called_once_with(BaseAlias.UBUNTU_JAMMY.value)
